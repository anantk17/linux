				     ...
				     int type)
{
	...
	if (!(auditd_test_task(current) || audit_ctl_owner_current())) {
		...
		while (audit_backlog_limit &&
			...
			//REMOVE_THIS_PRINTK
			pr_notice("Trying to signal kauditd from log_start for process id\n");
			wake_up_interruptible(&kauditd_wait);

			pr_notice("Check value : %d && %ld",gfpflags_allow_blocking(gfp_mask),stime);
			...
			if (gfpflags_allow_blocking(gfp_mask) && (stime > 0)) {
				...
				add_wait_queue_exclusive(&audit_backlog_wait,
							 &wait);
				//REMOVE_THIS_PRINTK
				pr_notice("Going to sleep for time , blocking process id\n");
...
void audit_log_end(struct audit_buffer *ab)
{
	...
	if (audit_rate_check()) {
		...
		//REMOVE_THIS_PRINTK
		pr_notice("sending a new audit message, signalling kauditd_thread for process id, type : %d",nlh->nlmsg_type);
		wake_up_interruptible(&kauditd_wait);
	} else
